...
import { FullscreenExitTwoTone, LocalConvenienceStoreOutlined } from '@material-ui/icons';
...
const addPersonalBest = async (name,result,setError) => {
    ...
    const ref = firestore.collection("users").doc(uid).collection(`personalBests`);
    ...
    const isExcerciseAlreadyInDatabase = (currentResults.docs.some(doc=>doc.id==name))
...
function PersonalBestsPage(){
  ...
  return <Container maxWidth ='lg'>
        ...
        <form className={`${classes.root} ${classes.form}`} noValidate autoComplete="off">
            <TextField value={exerciseName} name="exerciseName" id="exercise-input" label="Exercise Name" onChange={handleChange}/>
            <TextField InputProps={{
            endAdornment: <InputAdornment position="end">Kg</InputAdornment>,
          }} value={exerciseResult} name="exerciseResult"  id="exercise-result" label="Result"  onChange={handleChange}/>
          <Button type="button" variant="contained" color="primary" onClick={handleClick}>Add</Button>
        </form> 
