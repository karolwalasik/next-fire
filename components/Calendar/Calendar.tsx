...
function Calendar(props) {
  ...
  const retrieveData = async () => {
    ...
    let ref = await firestore
      .collection(`users/${props.authUser}/activities`)
      .onSnapshot((snpashot) => {
        const activities = snpashot.docs.filter(
          (snap) => snap.data().date === queryDate
        );
        console.log(activities, "ACTIV");
        //@ts-ignore
    ...
    await firestore
      ...
      .onSnapshot((snpashot) => {
        const arr = snpashot.docs.map((snap) => {
          if (snap.data().date) {
            return snap.data().date.length === 8
              ? snap.data().date.slice(0, 3)
              : snap.data().date.slice(0, 4);
          }
        });
        console.log("arr", arr);
        setActiveDays(arr);
      ...
      <Grid item xs={12} md={7}>
        <Paper className="paper">
          ...
          <ActivityList
            ...
            isOnClientPage={props.isOnClientPage}
          />
        </Paper>
      </Grid>
      <Snackbar
