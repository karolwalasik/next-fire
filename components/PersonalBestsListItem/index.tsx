...
function PersonalBestsListItem({name}){
    ...
    const removePersonalBest = async(name) =>{
        ...
        const ref = firestore.collection("users").doc(uid).collection(`personalBests`).doc(name);
        await ref.delete()
    ...
    <Accordion onChange={handleExpanded}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} style={{justifyContent: 'space-between'}}>
